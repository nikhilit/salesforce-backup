...
import caseSynctoSAP from '@salesforce/apex/AgentAttendanceController.caseSynctoSAP';
export default class AttendanceTracker extends NavigationMixin(LightningElement) {
...
@track isBackOffice = false;
    ...
    try {
        ...
        this.isAdmin = profileName && profileName.toLowerCase().includes('admin');

        this.typeOfWork = blockInfo.typeOfWork;
        this.isFrontOffice = this.typeOfWork === 'Front Office' && !isCRMAdmin;
        this.isBackOffice = this.typeOfWork === 'Back Office';

        // Map and apply prefixes
        let allBlocks = blockInfo.blockOptions.map(block => ({
            label: this.addMGLPrefix(block),
            value: block
        }));

        // Apply filtering logic based on typeOfWork
        if (this.typeOfWork === 'Front Office') {
            this.blockOptions = allBlocks.filter(
                block =>
                    block.label !== 'MGL Call Center' &&
                    block.label !== 'MGL Back Office'
            );
        } else if (this.typeOfWork === 'Call Center') {
            this.blockOptions = allBlocks.filter(
                block => block.label === 'MGL Call Center'
            );
        } else if (this.typeOfWork === 'Back Office') {
            this.blockOptions = allBlocks.filter(
                block => block.label === 'MGL Back Office'
            );
        } else {
            this.blockOptions = allBlocks; // fallback
        }
...
async handleCaseSync() {
    this.isLoading = true;
    try {
        await caseSynctoSAP();
        this.showSuccess('Sync started.');
    } catch (error) {
        this.showError('Sync Failed', this.getErrorMessage(error));
    } finally {
        this.isLoading = false;
    }
}
