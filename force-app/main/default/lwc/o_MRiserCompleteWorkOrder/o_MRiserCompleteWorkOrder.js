...
const LIST_VIEW_API_NAME = 'O_MCompleteWorkOrder';

const GET_COMPLETED_WORKORDERS = gql`
query getCompletedWorkOrders($userId: ID!) {
  uiapi {
    query {
      WorkOrder(
        where: {
          Appointment_Status__c: { eq: "Completed" },
          OwnerId: { eq: $userId }
        },
...
export default class O_MRiserCompleteWorkOrder extends NavigationMixin(LightningElement) {
    @track workOrders = [];
    ...
    @wire(graphql, { query: GET_COMPLETED_WORKORDERS, variables: '$graphQLVariables' })
    wiredWorkOrders({ error, data }) {
        if (data) {
            ...
            console.log(`✅ Loaded ${this.workOrders.length} Completed Work Orders`);
        } else if (error) {
            this.isLoading = false;
