...
import getObjectList from '@salesforce/apex/AS_CaseOMCSVUploaderController.getObjectList';
import uploadCSVBatch from '@salesforce/apex/AS_CaseOMCSVUploaderController.uploadCSVBatch';
import uploadLargeCSVChunk from '@salesforce/apex/AS_CaseOMCSVUploaderController.uploadLargeCSVChunk';

export default class AsOMCaseCSVUploader extends NavigationMixin(LightningElement) {
    ...
    handleApprovalStatusChange(event){
        ...
        console.log('Inside handleApprovalStatusChange method');

        this.approvedOrRejectedValue = event.target.value;
    }
...
formatDateForSalesforce(date) {
    ...
    validateCaseCSVHeaders(headers) {
        ...
        if (missing.length > 0) {
            this.showToast('Missing Required Column', `The following required column is missing: ${missing.join(', ')}. Required column: Docket_Number__c. Optional columns: Standard_Remarks__c, Planned_Start_date__c, Planned_End_Date__c, Status`, 'error');
            return false;
        }
         return true;
    }
    ...
    async handleUpload() {
       ...
       if(this.approvedOrRejectedValue==''){
        console.log('inside approval status empty');
         
           await LightningAlert.open({
                message: 'The Approval Status field is required',
                theme: 'warning',
                label: 'Warning'
            });     
                ...
                while (uploaded < total) {
                    ...
                    try {
                        const res = await uploadCSVBatch({
                            objectName: this.selectedObject,
                            approvalStatus : this.approvedOrRejectedValue,
                            csvRecords: chunk
                        });
