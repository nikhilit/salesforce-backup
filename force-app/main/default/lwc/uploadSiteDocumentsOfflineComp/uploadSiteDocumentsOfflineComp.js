 ...
 /* getWorkStepStatus(){
    ...
    wiredWorkStep({ data, error }) {
        if (data) {
            ...
            // Filter by Name
            const ws = data.records.find(r => r.fields.Name.value === 'Upload Site Documents');
            if (ws) {
...
wiredWorkSteps({ data, error }) {
    if (data) {
        ...
        const riserStep = records.find(r => r.fields.Name.value === 'Riser activity Possible');
...
wiredWorkOrder({ data, error }) {
    ...
    @track documentTypes = [
        ...
        { label: 'PTW', fileName: [], file: [], base64: [] }
