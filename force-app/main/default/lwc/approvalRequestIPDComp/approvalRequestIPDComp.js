...
export default class ApprovalRequestIPDComp extends LightningElement {
    ...
    { label: 'Record Name', fieldName: 'recordLink', type: 'url', 
      ...
      { label: 'BP Number', fieldName: 'BPNumber'   },

       { label: 'Documnet Type', fieldName: 'ApprovalProcessName'   },
   ...
   // @track tabName='IndentOrder';
    ...
    handleBulkAction(action) {
        ...
        if (selectedRows.length === 0) {
            this.showToastMessage('Error', 'No records selected', 'error');
            return;
        }
        ...
        handleApproval({ workItemIds, action })
            .then(() => {
                this.showToastMessage('Success', `Requests ${action}d successfully`, 'success');
                //return refreshApex(this.approvalRequests);
                

            })
            .catch((error) => {
                this.showToastMessage('Error', `Failed to ${action} requests: ${error.body.message}`, 'error');
            });
    }
