...
export default class UploadSiteDocumentsICCommericalComp extends NavigationMixin(LightningElement){
...
@track previewIsImage = false;
    ...
    // startFlow() {
    //     const url = `com.salesforce.fieldservice://v1/sObject/${this.recordId}/quickaction/IC_Upload_File`;
...
//     @track documentTypes = [
//     { label: "HIRA (Hazard Identification and Risk Assessment)", files: [] },
//     { label: "Hot Work Permit", files: [] },
//     { label: "Others", files: [] }
// ];
@track documentTypes = [
    { label: "HIRA (Hazard Identification and Risk Assessment)", files: [], counter: 0 },
    { label: "Hot Work Permit", files: [], counter: 0 },
    { label: "Others", files: [], counter: 0 }
];

  // each doc will have uploaded, previewUrl, fileName
    @track files = [];
    ...
    wiredSteps({ data, error }) {
        ...
        if (data && data.records && data.records.length > 0) {
            ...
            if (wsName === 'uploaddocuments') {
                this.workstepsId = r.fields.Id.value;
                console.log('WorkStep ID upload documents:', this.workstepsId);
            }
    ...
    const MAX_IMAGES = 6;
        ...
        // Correct limit check (before adding)
    ...
    newFiles.forEach(async file => {
        ...
        // Compress ONLY if image and size > 2MB
        if (file.type.startsWith("image/") && file.size > 2 * 1024 * 1024) {
            finalFile = await this.compressImage(file, 2 * 1024 * 1024);
        }
...
//     closePreview() {
//     if (this.previewIsPDF && this.previewUrl.startsWith('blob:')) {
